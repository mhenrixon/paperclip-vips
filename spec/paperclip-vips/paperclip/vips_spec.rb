RSpec.describe PaperclipVips do
  it "has a version number" do
    expect(PaperclipVips::VERSION).not_to be_nil
  end

  describe "crop" do
    let(:file) do
      path = File.join(described_class.root, "spec", "support", "image.jpg")
      File.new(path, "rb")
    end
  ...
  describe "current_format" do
    it "returns format if path includes extension" do
      ...
      File.write(file.path, File.read(File.join(described_class.root, "spec", "support", "image.jpg")))
    ...
    it "returns format if path excludes extension" do
      string_io = StringIO.open(File.read(File.join(described_class.root, "spec", "support", "image.jpg")))
  ...
  describe "gif images" do
    let(:file) do
      path = File.join(described_class.root, "spec", "support", "image.gif")
      File.new(path, "rb")
    end
  ...
  describe "png images" do
    let(:file) do
      path = File.join(described_class.root, "spec", "support", "image.png")
      File.new(path, "rb")
    end
